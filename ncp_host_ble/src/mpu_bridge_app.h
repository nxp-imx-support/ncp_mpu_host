...
/**
 *@brief check uuid string is valid or not
 *
 *@param uuid         A pointer string buffer of uuid
 *@return             1 - success   0 - failure
 **/
uint8_t uuid_str_valid(const char *uuid);

/**
 *@brief convert uuid char to hex array
 *
 *@param[in] uuid     A pointer string buffer of uuid
 *@param[out] arry    A pointer array buffer of uuid
 *@param[out] type    uuid type
 *@return             1 - success   0 - failure
 **/
uint8_t uuid2arry(const char *uuid, uint8_t *arry, uint8_t type);

/**
 *@brief Send the TLV command to device
 *
 *@param S_D          A pointer of data to send
 **/
