...
{
    NCPCmd_DS_COMMAND *start_adv_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)start_adv_command, 0, NCP_BRIDGE_COMMAND_LEN);

    start_adv_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_START_ADV;
    start_adv_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    start_adv_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    start_adv_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_set_adv_data_command(int argc, char **argv)
{
    ...
    NCPCmd_DS_COMMAND *set_adv_data_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_adv_data_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    set_adv_data_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_SET_ADV_DATA;
    set_adv_data_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_adv_data_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_adv_data_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *stop_adv_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)stop_adv_command, 0, NCP_BRIDGE_COMMAND_LEN);

    stop_adv_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_STOP_ADV;
    stop_adv_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    stop_adv_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    stop_adv_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *set_scan_para_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_scan_para_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    set_scan_para_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_SET_SCAN_PARAM;
    set_scan_para_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_scan_para_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_scan_para_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    int scan_type;
    NCPCmd_DS_COMMAND *start_scan_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)start_scan_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    start_scan_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_START_SCAN;
    start_scan_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    start_scan_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    start_scan_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *stop_scan_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)stop_scan_command, 0, NCP_BRIDGE_COMMAND_LEN);

    stop_scan_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_STOP_SCAN;
    stop_scan_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    stop_scan_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    stop_scan_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
void ble_connect_command_local(MCP_CMD_CONNECT *param)
{
    ...
    (void)memset((uint8_t *)connect_command, 0, NCP_BRIDGE_COMMAND_LEN);
    connect_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_CONNECT;
...
int ble_disconnect_command(int argc, char **argv)
{
    ...
    (void)memset((uint8_t *)disconnect_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    disconnect_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_DISCONNECT;
...
int ble_set_data_len_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *set_data_len_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_data_len_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    set_data_len_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_SET_DATA_LEN;
    set_data_len_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_data_len_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_data_len_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_set_phy_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *set_phy_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_phy_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    set_phy_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_SET_PHY;
    set_phy_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_phy_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_phy_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_connect_paramter_update_command(int argc, char **argv)
{
    ...
    NCPCmd_DS_COMMAND *conn_param_update_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)conn_param_update_command, 0, NCP_BRIDGE_COMMAND_LEN);

    conn_param_update_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_CONN_PARAM_UPDATE;
    conn_param_update_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    conn_param_update_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    conn_param_update_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_set_filter_list_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *set_filter_list_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_filter_list_command, 0, NCP_BRIDGE_COMMAND_LEN);

    set_filter_list_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_SET_FILTER_LIST;
    set_filter_list_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_filter_list_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_filter_list_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
    ...
    ((uint8_t *)set_filter_list_command)[sizeof(NCP_BRIDGE_COMMAND)]= atoi(argv[1]);
    for (i = 0; i< atoi(argv[1]); i++)
    {
      ...
      ((uint8_t *)set_filter_list_command)[sizeof(NCP_BRIDGE_COMMAND) + 1 + (1 + 6) * i] = type;
      memcpy((uint8_t *)(&(((uint8_t *)set_filter_list_command)[sizeof(NCP_BRIDGE_COMMAND) + 2 + (1 + 6) * i])), raw_addr, NCP_BLE_ADDR_LENGTH);
    }
...
int ble_start_encryption_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *start_encryption_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)start_encryption_command, 0, NCP_BRIDGE_COMMAND_LEN);

    start_encryption_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GAP_PAIR;
    start_encryption_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    start_encryption_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    start_encryption_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *write_characteristic_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)write_characteristic_command, 0, NCP_BRIDGE_COMMAND_LEN);

    write_characteristic_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_SET_VALUE;
    write_characteristic_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    write_characteristic_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    write_characteristic_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_read_characteristic_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *read_characteristic_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)read_characteristic_command, 0, NCP_BRIDGE_COMMAND_LEN);

    read_characteristic_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_READ;
    read_characteristic_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    read_characteristic_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    read_characteristic_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_register_service_command(int argc, char **argv)
{
    ...
    NCPCmd_DS_COMMAND *register_service_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)register_service_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    register_service_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_REGISTER_SERVICE;
    register_service_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    register_service_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    register_service_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *set_power_mode_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_power_mode_command, 0, NCP_BRIDGE_COMMAND_LEN);

    set_power_mode_command->header.cmd      = NCP_BRIDGE_CMD_BLE_VENDOR_POWER_MODE;
    set_power_mode_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_power_mode_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_power_mode_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_set_device_address_command(int argc, char **argv)
{
    int ret;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *set_device_address_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_device_address_command, 0, NCP_BRIDGE_COMMAND_LEN);

    set_device_address_command->header.cmd      = NCP_BRIDGE_CMD_BLE_VENDOR_SET_DEVICE_ADDR;
    set_device_address_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_device_address_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_device_address_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    int len;
    NCPCmd_DS_COMMAND *set_device_name_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)set_device_name_command, 0, NCP_BRIDGE_COMMAND_LEN);

    set_device_name_command->header.cmd      = NCP_BRIDGE_CMD_BLE_VENDOR_SET_DEVICE_NAME;
    set_device_name_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    set_device_name_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    set_device_name_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *start_service_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)start_service_command, 0, NCP_BRIDGE_COMMAND_LEN);
    ...
    // {
    //     start_service_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_START_SERVICE;
    //     start_service_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    //     start_service_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    //     start_service_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
    // }
...
{
    NCPCmd_DS_COMMAND *ble_disc_prim_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)ble_disc_prim_command, 0, NCP_BRIDGE_COMMAND_LEN);

    ble_disc_prim_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_DISC_PRIM;
    ble_disc_prim_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    ble_disc_prim_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    ble_disc_prim_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *ble_disc_chrc_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)ble_disc_chrc_command, 0, NCP_BRIDGE_COMMAND_LEN);

    ble_disc_chrc_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_DISC_CHRC;
    ble_disc_chrc_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    ble_disc_chrc_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    ble_disc_chrc_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *ble_disc_desc_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)ble_disc_desc_command, 0, NCP_BRIDGE_COMMAND_LEN);

    ble_disc_desc_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_DESC_CHRC;
    ble_disc_desc_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    ble_disc_desc_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    ble_disc_desc_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_cfg_subscribe_command(int argc, char **argv)
{
    ...
    uint8_t addr_type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *ble_subcribe_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)ble_subcribe_command, 0, NCP_BRIDGE_COMMAND_LEN);

    ble_subcribe_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    ble_subcribe_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    ble_subcribe_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
    ...
    if (!strncmp(argv[1], "indicate", 9))
        ble_subcribe_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_CFG_INDICATE;
    else if (!strncmp(argv[1], "notify", 7))
        ble_subcribe_command->header.cmd      = NCP_BRIDGE_CMD_BLE_GATT_CFG_NOTIFY;
    else
    {
...
int ble_l2cap_connection_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *l2cap_connect_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)l2cap_connect_command, 0, NCP_BRIDGE_COMMAND_LEN);

    l2cap_connect_command->header.cmd      = NCP_BRIDGE_CMD_BLE_L2CAP_CONNECT;
    l2cap_connect_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    l2cap_connect_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    l2cap_connect_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_l2cap_disconnect_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *l2cap_disconnect_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)l2cap_disconnect_command, 0, NCP_BRIDGE_COMMAND_LEN);

    l2cap_disconnect_command->header.cmd      = NCP_BRIDGE_CMD_BLE_L2CAP_DISCONNECT;
    l2cap_disconnect_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    l2cap_disconnect_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    l2cap_disconnect_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_l2cap_send_command(int argc, char **argv)
{
    ...
    uint8_t type;
    uint8_t raw_addr[NCP_BLE_ADDR_LENGTH];
    NCPCmd_DS_COMMAND *l2cap_send_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)l2cap_send_command, 0, NCP_BRIDGE_COMMAND_LEN);

    l2cap_send_command->header.cmd      = NCP_BRIDGE_CMD_BLE_L2CAP_SEND;
    l2cap_send_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    l2cap_send_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    l2cap_send_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *l2cap_register_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)l2cap_register_command, 0, NCP_BRIDGE_COMMAND_LEN);

    l2cap_register_command->header.cmd      = NCP_BRIDGE_CMD_BLE_L2CAP_REGISTER;
    l2cap_register_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    l2cap_register_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    l2cap_register_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *l2cap_rmetrics_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)l2cap_rmetrics_command, 0, NCP_BRIDGE_COMMAND_LEN);

    l2cap_rmetrics_command->header.cmd      = NCP_BRIDGE_CMD_BLE_L2CAP_METRICS;
    l2cap_rmetrics_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    l2cap_rmetrics_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    l2cap_rmetrics_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
{
    NCPCmd_DS_COMMAND *l2cap_receive_command = ncp_mpu_bridge_get_command_buffer();
    (void)memset((uint8_t *)l2cap_receive_command, 0, NCP_BRIDGE_COMMAND_LEN);

    l2cap_receive_command->header.cmd      = NCP_BRIDGE_CMD_BLE_L2CAP_RECEIVE;
    l2cap_receive_command->header.size     = NCP_BRIDGE_CMD_HEADER_LEN;
    l2cap_receive_command->header.result   = NCP_BRIDGE_CMD_RESULT_OK;
    l2cap_receive_command->header.msg_type = NCP_BRIDGE_MSG_TYPE_CMD;
...
int ble_process_ncp_event(uint8_t *res)
{
    ...
    {
        case NCP_BRIDGE_EVENT_IUT_READY:
            ret = WM_SUCCESS;
            printf("IUT Device is ready!\r\n");
            break;
        case NCP_BRIDGE_EVENT_ADV_REPORT:
            ret = ble_process_adv_report(res);
            break;
        case NCP_BRIDGE_EVENT_DEVICE_CONNECTED:
            ret = ble_process_device_connected(res);
            break;
        case NCP_BRIDGE_EVENT_DEVICE_DISCONNECT:
            ret = ble_process_device_disconnected(res);
            break;
        case NCP_BRIDGE_EVENT_PHY_UPDATED:
            ret = ble_process_phy_update(res);
            break;
        case NCP_BRIDGE_EVENT_DATA_LEN_UPDATED:
            ret = ble_process_data_len_update(res);
            break;
        case NCP_BRIDGE_EVENT_PASSKEY_DISPLAY:
            ret = ble_process_passkey_display(res);
            break;
        case NCP_BRIDGE_EVENT_IDENITY_RESOLVED:
            ret = ble_process_idenity_resolved(res);
            break;
        case NCP_BRIDGE_EVENT_CONN_PARAM_UPDATE:
            ret = ble_process_conn_param_update(res);
            break;
        case NCP_BRIDGE_EVENT_SEC_LEVEL_CHANGED:
            ret = ble_process_security_level_changed(res);
            break;
        case NCP_BRIDGE_EVENT_GATT_NOTIFICATION:
            ret = ble_process_gatt_notification(res);
            break;
        case NCP_BRIDGE_EVENT_ATTR_VALUE_CHANGED:
            ret = ble_process_attr_value_changed(res);
            break;
        case NCP_BRIDGE_EVENT_GATT_CCC_CFG_CHANGED:
            ret = ble_process_gatt_ccc_cfg_changed(res);
            break;
        case NCP_BRIDGE_EVENT_GATT_SUBSCRIPTIONED:
            ret = ble_process_gatt_subscriptioned(res);
            break;
        case NCP_BRIDGE_EVENT_L2CAP_CONNECT:
            ret = ble_process_l2cap_connected(res);
            break;
        case NCP_BRIDGE_EVENT_L2CAP_DISCONNECT:
            ret = ble_process_l2cap_disconnected(res);
            break;
        case NCP_BRIDGE_EVENT_L2CAP_RECEIVE:
            ret = ble_process_l2cap_received(res);
            break;
        case NCP_BRIDGE_EVENT_GATT_DISC_PRIM:
            ret = ble_process_gatt_prim_discovered(res);
            break;
        case NCP_BRIDGE_EVENT_GATT_DISC_CHRC:
            ret = ble_process_gatt_chrc_discovered(res);
            break;
        case NCP_BRIDGE_EVENT_GATT_DISC_DESC:
...
int ble_process_l2cap_received(uint8_t *res)
{
    ...
    dump_hex(l2cap_received_tlv->data, l2cap_received_tlv->len);
...
int ble_process_register_service_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_BRIDGE_CMD_RESULT_OK)
    {
        ...
        for(int i = 0; i < svc_length; i++) {
            ...
            if (register_sev_rp->service[i] == NCP_BRIDGE_CMD_RESULT_OK)
            {
                printf("Success\r\n");
            }else {
                printf("Failed\r\n");
